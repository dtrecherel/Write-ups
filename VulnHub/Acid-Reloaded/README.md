...
I can access to the webpage with the URL `http://172.16.130.131:33447/`.

![2]

*Brain - Use it*

I couldn't find anything interesting in the source code so I launched `dirbuster` and ran a scan to find more directories & files.

![3]

I found the `/bin` directory, which displays a login page. I tried to do some SQLi but it didn't work.

![4]

## HTTP Headers

I also found the page `/bin/dashboard.php`, which tells me that I'm not authorized to access the page.

![5]

After trying for a while SQLi on the login page, I tried to play with HTTP Headers. I assumed that when trying to log in, we're redirected to `/bin/includes/validation.php` and then to `/bin/dashboard.php` (if the login is successfull).

I tried to connect to `/bin/dashboard.php`, but interepted the request and added `/bin/includes/validation.php` as the referer.
...
I can extract the data archived in that file
